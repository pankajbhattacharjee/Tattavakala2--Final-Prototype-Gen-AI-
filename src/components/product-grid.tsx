...
export default function ProductGrid({ searchQuery = '', filters }: ProductGridProps) {
  ...
  useEffect(() => {
    ...
    setAllProducts(prev => {
      // Deduplicate by product ID, prioritizing Firestore > localStorage > initial
      const productMap = new Map<string, Product>();

      // Add initial products first (lowest priority)
      combinedProducts.forEach(p => productMap.set(p.id, p));

      // Add localStorage products (medium priority)
      storedProducts.forEach(p => productMap.set(p.id, p));

      // Add Firestore products (highest priority)
