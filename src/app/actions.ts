...
async function callGenkitFlow<T_IN, T_OUT>(flowId: string, input: T_IN): Promise<T_OUT> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Genkit flow '${flowId}' failed with status ${response.status}:`, errorBody);
...
export async function generateProductStory(
  ...
  return await callGenkitFlow('generateProductStoryFlow', input);
}
...
export async function speechToText(
  ...
  return await callGenkitFlow('speechToTextFlow', input);
}
